    ...
    <header class="header">
      <div class="wide-container header-content">
        ...
        <div class="nav-area">
          <nav class="nav">
            <!-- 首页 -->
            <router-link :to="localePath('/')" class="nav-item home-nav-item" :class="{ 'active-nav': isActive('/') }">{{ t('nav.home') }}</router-link>
            ...
            >
              <router-link 
                ...
                class="nav-item"
                :class="{ 'has-submenu': menu.children?.length, 'active-nav': isActive(menu.path) }"
              >
                {{ t(`nav.${menu.key}`) }}
